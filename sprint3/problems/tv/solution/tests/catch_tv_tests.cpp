...
SCENARIO("Controller", "[Controller]") {
    ...
    GIVEN("Controller and TV") {
        ...
        WHEN("The TV is turned off") {
            ...
            AND_WHEN("SelectChannel command is entered with 12 channel number then TV turned off") {
            ...
            AND_WHEN("SelectPreviousChannel command then TV turned off") {
...
//#endif
            ...
            AND_WHEN("Info command is entered with some arguments") {
                ...
                THEN("the error message is printed and the Info command does not require any arguments") {
                    expect_output("Error: the Info command does not require any arguments\n"s);
                }
            }

            AND_WHEN("SelectChannel command is entered with 12 channel number") {
                run_menu_command("SelectChannel 12"s);
            ...
            AND_WHEN("SelectChannel command is entered without arguments") {
                ...
                THEN("the error message is printed and 'empty argument' message") {
                    expect_output("Error: Empty argument.\n"s);
                }
            }

            AND_WHEN("SelectChannel command is entered with several arguments") {
                run_menu_command("SelectChannel 12 13"s);

                THEN("the error message is printed and 'Invalid number of arguments' message") {
                    expect_output("Error: Invalid number of arguments.\n"s);
                }
            }
            ...
            AND_WHEN("SelectChannel command is entered with 12 channel number") {
                ...
                THEN("the error message is printed") {
                    expect_output("Error: Channel is out of range.\n"s);
                }
            }

            AND_WHEN("SelectChannel command is entered with no interger type channel number") {
            ...
            AND_WHEN("SelectPreviousChannel command enterd") {
                tv.SelectChannel(13);
                run_menu_command("SelectPreviousChannel"s);
            ...
            AND_WHEN("SelectPreviousChannel command with some arguments") {
                run_menu_command("SelectPreviousChannel 12"s);

                THEN("the error message is printed and the SelectPreviousChannel command does not require any arguments") {
                    expect_output("Error: the SelectPreviousChannel command does not require any arguments\n"s);
                }
            }

            AND_WHEN("SelectPreviousChannel command is entered with current channel with minimum number") {
                run_menu_command("SelectPreviousChannel"s);
