...
Position Dog::CalculateNewPosition(const std::chrono::milliseconds& delta_time) {
    ...
    position.x += velocity.vx * (static_cast<double>(delta_time.count()) / MILLISECONDS_IN_SECOND);
    position.y += velocity.vy * (static_cast<double>(delta_time.count()) / MILLISECONDS_IN_SECOND);
    return position;
};
