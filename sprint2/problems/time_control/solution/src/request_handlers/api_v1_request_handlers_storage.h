...
const std::set<std::string_view> GAME_API_URLS = {
    "/api/v1/game/players",
    "/api/v1/game/players/",
    "/api/v1/game/state",
    "/api/v1/game/state/",
...
const std::set<std::string_view> GAME_API_URLS_WITH_JSON_REQ = {
    "/api/v1/game/join",
    "/api/v1/game/join/",
    "/api/v1/game/player/action",
    "/api/v1/game/player/action/"
...
std::optional<size_t> GetMapListHandler(
    ...
    response.set(http::field::content_type, CONTENT_TYPE_APPLICATION_JSON);
    response.set(http::field::cache_control, req[http::field::cache_control]);
...
bool EmptyAuthorizationActivator(const Request& req) {
    return (GAME_API_URLS.count(req.target()) > 0) &&
