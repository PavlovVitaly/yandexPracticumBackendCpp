...
void Application::BoundPlayerAndGameSession(std::shared_ptr<model::Player> player,
    ...
    player->SetDog(session->CreateDog(player->GetName(), randomize_spawn_points_));
};

const std::vector< std::weak_ptr<model::Player> >& Application::GetPlayersFromGameSession(const authentication::Token& token) {
